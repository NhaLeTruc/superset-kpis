...
class TestAnomalyDetection:
    ...
    def test_statistical_anomaly_detection(self, spark):
        ...
        data.append(("int_outlier_1", "user_outlier_1", "edit", datetime(2024, 1, 1), 1000))
        data.append(("int_outlier_2", "user_outlier_2", "save", datetime(2024, 1, 1), 2000))
        data.append(("int_outlier_3", "user_outlier_3", "close", datetime(2024, 1, 1), 5000))

        df = spark.createDataFrame(data, schema)

        # Detect anomalies with z-score threshold of 3.0
        anomalies = detect_anomalies_statistical(df, value_column="duration_ms", z_threshold=3.0)
    ...
    def test_grouped_anomaly_detection(self, spark):
        ...
        for i in range(50):
            data.append(
                (f"int_a_{i}", f"user_a_{i}", "open", datetime(2024, 1, 1), 100 + (i % 100), "A")
            )
        data.append(("int_a_outlier", "user_a_outlier", "edit", datetime(2024, 1, 1), 1000, "A"))

        # Group B: Normal range 500-600ms, with one outlier at 2000ms
        for i in range(50):
            data.append(
                (f"int_b_{i}", f"user_b_{i}", "save", datetime(2024, 1, 1), 500 + (i % 100), "B")
            )
        data.append(("int_b_outlier", "user_b_outlier", "close", datetime(2024, 1, 1), 2000, "B"))

        df = spark.createDataFrame(data, schema)

        # Detect anomalies within groups
        anomalies = detect_anomalies_statistical(
            df, value_column="duration_ms", z_threshold=3.0, group_by_columns=["group"]
        )
